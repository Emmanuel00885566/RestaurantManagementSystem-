...
export const updateStockLevel = (menuId, quantityUsed) => {
  ...
  if (item.quantity <= item.lowStockThreshold) {
    console.log(`⚠️ Low stock alert: ${item.name} (${item.quantity} ${item.unit} left)`);
  }
...
export const restoreStockLevel = (menuId, quantityToRestore) => {
  ...
  console.log(`🔄 Restored ${quantityToRestore} units of ${item.name}. New total: ${item.quantity} ${item.unit}`);
  return item;
};